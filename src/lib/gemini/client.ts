...
KLIENTI vs. LEADY — POVINNÉ ROZLIŠENÍ:
"noví klienti" / "přehled klientů" / "kolik klientů" / "klientská základna" → query_client_metrics (tabulka clients)
...
Nikdy negeneruj pseudo-grafy pomocí znaků █, ▇, ▆, ░, ▒, ▓ nebo jiných blokových/čárových symbolů v textu zprávy.
Pokud uživatel chce grafické znázornění, vložíš ho jako chart artifact — server vizualizaci vykreslí automaticky.
Textová zpráva obsahuje pouze slovní shrnutí, ne žádnou ASCII nebo Unicode vizualizaci.

COMPOUND ANALYTICKÉ DOTAZY:
Pokud dotaz obsahuje více dimenzí pro jednu entitu (např. "kolik přišlo za Q1 a odkud?"), zavolej příslušný tool DVAKRÁT — jednou s groupBy="month" pro vývoj v čase, jednou s groupBy="source" pro breakdown zdrojů.
Oba grafy se zobrazí jako dvě karty v UI — nevybírej jen jeden.
...
export const BUSINESS_FUNCTION_DECLARATIONS: FunctionDeclaration[] = [
  {
    ...
    description:
      "Získá počty leadů (poptávek/zápisů zájmu o nemovitost) z tabulky leads. Použij POUZE pro dotazy na leady, poptávky nebo zájemce — NE pro dotazy na 'klienty'. Pro otázky 'odkud přišli leady' použij groupBy='source'. Pro vývoj za měsíce použij groupBy='month'.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        dateRange: {
  ...
  },
  {
    ...
    description:
      "Získá počty klientů z tabulky clients podle období a seskupení. Použij pro dotazy na 'noví klienti', 'kolik klientů', 'přehled klientů', 'klientská základna'. Pro otázky 'odkud přišli klienti' použij groupBy='source'. Pro vývoj za měsíce použij groupBy='month'.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        dateRange: {
