...
function WorkspaceOutline() {
  return (
    ...
    >
      ...
      <path d="M58 382h314c35 0 58-9 80-28" stroke="#151515" strokeWidth="2" />
    </svg>
  );
}
...
function LoginForm() {
  ...
  async function handleGoogleLogin() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/chat/new`,
    ...
    <div className="flex w-full max-w-[390px] flex-col">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 py-1">
          ...
          <span className="text-[13px] text-[#536471]">nebo</span>
          <div className="h-px flex-1 bg-[#2f3336]" />
        </div>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-3">
          ...
          <button
            type="submit"
            disabled={anyLoading}
            className="flex h-12 w-full items-center justify-center gap-3 rounded-md border border-[#2f3336] bg-transparent px-5 text-[15px] font-semibold text-[#e7e9ea] transition hover:bg-[#16181c] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {loading ? (
